...
public final class Layout {
    ...
    /** Splits an area into <count> equal-width rectangles with a fixed gap, centered horizontally. */
    public static List<Rectangle> distributeHorizontally(Rectangle area, int count, int gap) {
        List<Rectangle> rects = new ArrayList<>(Math.max(0, count));
        if (count <= 0 || area.width <= 0) return rects;
    ...
    /** Legacy card row helper (kept for back-compat). */
    public static List<Rectangle> cardRow(Rectangle container, int padding, int y, int height, int count, int gap) {
        ...
        int available = Math.max(0, container.width - padding * 2);
        int totalGap = gap * Math.max(0, count - 1);
    ...
    /** Scales cards to always fit a single row inside {@code area}, preserving gaps. */
    public static List<Rectangle> oneRowCardBounds(int count, int cardWidth, int gap, Rectangle area) {
        List<Rectangle> rects = new ArrayList<>(Math.max(0, count));
        if (count <= 0 || area.width <= 0 || area.height <= 0) return rects;
